...
public class OtpService(IRedisService redisService,
    IBaseRepository<Otp> otpRepository,
    IEskizService eskizService) : IOtpService
{
    private readonly IRedisService _redisService = redisService;
    private readonly IBaseRepository<Otp> _otpRepository = otpRepository;
    private readonly IEskizService _eskizService = eskizService;
    ...
    public async Task<string> SendSMSAsync(string toPhoneNumber)
    {
        int code = new Random().Next(1111, 9999);
        await _redisService.SetItemAsync(toPhoneNumber, code);
        var (isSuccess, result) = await _eskizService.SendSmsAsync(phoneNumber: toPhoneNumber, code.ToString());
        if (isSuccess)
            return result;
        return string.Empty;
    }
